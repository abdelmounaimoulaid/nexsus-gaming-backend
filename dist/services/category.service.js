...
class CategoryService {
    static async getCategories(query) {
        ...
        if (query.noPagination === 'true') {
            const allCategories = await index_1.prisma.category.findMany({
                where,
                orderBy: { sortOrder: 'asc' },
        ...
        const [categories, total] = await Promise.all([
            index_1.prisma.category.findMany({
                where,
                skip,
                take: limit,
                orderBy: { sortOrder: 'asc' },
            ...
            index_1.prisma.category.count({ where })
        ]);
        return {
            data: categories,
            meta: {
    ...
    static async deleteCategory(id) {
        ...
        const getDescendantIds = async (parentId) => {
            const children = await index_1.prisma.category.findMany({ where: { parentId }, select: { id: true } });
        ...
        for (const catId of allCategoryIdsToDelete.reverse()) {
            await index_1.prisma.category.delete({ where: { id: catId } }).catch(() => { });
        }
        return { success: true };
    }
    static async exportCategoriesToExcel() {
        const all = await index_1.prisma.category.findMany({
    ...
    static async importCategories(file) {
        ...
        const existing = await index_1.prisma.category.findMany({ select: { id: true, slug: true } });
        ...
        for (const row of records) {
            ...
            try {
                if (slugToId.has(safeSlug)) {
                    upserted = await index_1.prisma.category.update({
                        where: { slug: safeSlug },
                        data: {
                            name: row.name,
                    ...
                    upserted = await index_1.prisma.category.create({
                        data: {
                            name: row.name,
                            nameFr: row.nameFr || null,
                            nameEn: row.nameEn || null,
        ...
        for (const row of records) {
            ...
            try {
                await index_1.prisma.category.update({ where: { id: childId }, data: { parentId } });
            }
            catch (e) {
